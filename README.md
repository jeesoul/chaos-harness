...
<p align="center"><strong>AI 项目智能体 — 项目理解 + 智能决策 + 流程控制</strong></p>
<p align="center"><em>v1.3.3 — 知识图谱 · Gate 状态机 · 硬拦截 · 数据驱动</em></p>
...
> 用 Gate 状态机给 AI 开发流程立规矩，用知识图谱让 AI 真正理解项目。

## 核心问题

AI Agent 辅助开发的核心问题是非确定性——跳过验证、绕过约束、幻觉式交付。自然语言提示词是软性建议，存在语义博弈空间。

Chaos Harness 将约束编码为 **Gate 状态机**——通过 Gate 分级执行 + Hooks 自动拦截 + 知识图谱项目理解 + 知识库数据驱动，消除灰色地带。

### 四大特性

| 特性 | 说明 |
|------|------|
| **项目理解** | Graphify 知识图谱：AST 提取 + 社区检测 + God Nodes 分析，AI 真正理解代码结构 |
...
└──────────┬──────────────────────────────┘
    ...
    ▼             ▼              ▼
┌─────────┐ ┌──────────────┐ ┌──────────┐
│ Graphify│ │ Gate 状态机  │ │ Dev-Intel│
│ 知识图谱│ │ 11 Gates     │ │ BM25引擎 │
│ AST提取 │ │ 10 验证器    │ │ 15 CSV库 │
└─────────┘ └──────────────┘ └──────────┘
    │             │              │
    └─────────────┴──────────────┘
                  ▼
┌─────────────────────────────┐
│ 27 Scripts · 16 Skills      │
│ 10 验证器 · 5 技术栈适配      │
└─────────────────────────────┘
```
...
│   └── ...
├── scripts/             # 31 个脚本（执行引擎）
│   ├── graphify-wrapper.mjs # Graphify 集成层
│   ├── graphify-init.mjs    # SessionStart 图谱生成
│   ├── graphify-context.mjs # PreToolUse 上下文注入
│   ├── graphify-update.mjs  # PostToolUse 增量更新
...
├── data/                # 15 个 CSV 知识库 + chaos-config.json
├── stacks/              # 5 个技术栈配置
├── tests/               # 8 个测试文件
...
└── .chaos-harness/      # 运行时状态
    ├── gates/           # Gate 定义 + 状态
    ├── graphify-config.json # 项目级 Graphify 配置
    └── state.json       # 会话状态
```
...
> **注意：** 所有迭代在 `v1.3.3` 分支进行，升级时不要切换到 main 或其他分支。

### 版本历史

| 版本 | 主要更新 |
|------|---------|
| **1.3.3** | **AI 项目智能体**：Graphify 知识图谱集成（AST 提取 + Leiden 社区检测 + God Nodes 分析）、配置驱动行为（enabled 强制模式）、增量更新、交互式可视化、16 Skills、31 Scripts、8 测试文件 |
...
v1.3.3 核心能力。基于 [Graphify](https://github.com/safishamsi/graphify) 的项目知识图谱，让 AI 真正理解代码结构。
...
| **手动模式** | `enabled: false` | 仅 `/chaos-harness:graphify` 命令时使用 |
...
/chaos-harness:graphify explain "X" # 解释节点
...
| PostToolUse (Write\|Edit) | 写/编辑后 | graphify-update.mjs + learning-update + project-pattern-writer + workflow-track + dev-intelligence |
