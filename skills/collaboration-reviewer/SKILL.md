...
```bash
# 查看能力评测状态
/eval-harness list capability
/eval-harness report <eval-id>
```

如果存在相关评测：
- pass@3 >= 0.9 → 评审重点关注架构和质量
- pass@3 < 0.9 → 评审重点关注失败原因
...
```bash
# 查看回归评测
/eval-harness list regression
```

如果有回归失败项 → 评审不通过，必须先修复。
...
超频模式下的评审自动使用快速确认模式，但保留 IL-TEAM001（最少 2 Agent）底线。
